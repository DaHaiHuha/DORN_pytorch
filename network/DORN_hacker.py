...
class FullImageEncoder(nn.Module):
    def __init__(self):
        ...
        self.global_pooling = nn.AvgPool2d(16, stride=16, padding=(8, 8))  # KITTI 16 16 padding补１
        self.dropout = nn.Dropout2d(p=0.5)
        # self.global_fc = nn.Linear(2048 * 3 * 4, 512)  # kitti 4x5 # 进2048 * 4 * 5，　出５１２
        self.global_fc = nn.Linear(2048 * 3 * 6, 512)  # kitti 4x5 # 进2048 * 4 * 5，　出５１２
        self.relu = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(512, 512, 1)  # 1x1 卷积 H,W, kernal_size
        # self.upsample = nn.UpsamplingBilinear2d(size=(32, 60))  # KITTI 49X65 NYU 33X45 上采样到特定的size Question?如何选取
        self.upsample = nn.UpsamplingBilinear2d(size=(45, 80))  # compatibale with 360, 640
    ...
    def forward(self, x):
        ...
        x2 = self.dropout(x1)
        # reduce the channel
        # x3 = x2.view(-1, 2048 * 3 * 4)  # kitti 4x5
        x3 = x2.view(-1, 2048 * 3 * 6)  # kitti 4x5
        # remap the x2 to x3
        x4 = self.relu(self.global_fc(x3))
        # print('# x4 size:', x4.size())
        x4 = x4.view(-1, 512, 1, 1)
        # print('# x4 size:', x4.size())
        x5 = self.conv1(x4)  # identity
...
class SceneUnderstandingModule(nn.Module):
    def __init__(self):
        ...
        self.concat_process = nn.Sequential(
            ...
            # nn.UpsamplingBilinear2d(size=(256, 480))
...
if __name__ == "__main__":
    model = DORN(pretrained=True)
    model = model.cuda()
    model.eval()
    # image = torch.randn(1, 3, 256, 480)
